...
def news_by_date_Crawling(URL, index):
    ...
    for rank in range(0, 50):
        ...
        f.write(str(title) + '>\n') # str(index * 50 + rank + 1) -> To index an article, there are 50 articles on one web page for each day.
        if (index * 50 + rank + 1) < TOTAL: # For delete last '\n'
            f.write(contents + '>\n')
        else:
            f.write(contents + '>')
        # '>' -> To split after crawling

# Function of crawling articles at 2020.08.01~2020.08.20(1000 articles)
def newsCrawling(URL):
    index = 0
    for day in range(20200801, 20200821):
        url_day = URL + str(day)
